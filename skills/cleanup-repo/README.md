...
| `linearWritebackDefault` | Seeds the yes/no default of the Step 10 Linear `Done` writeback prompt (`"yes"` / `"no"`); the interactive gate always stays, so it never auto-applies. Absent → `"no"`. | `"no"` |
