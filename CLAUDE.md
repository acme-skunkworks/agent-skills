...
The root package is `@acme-skunkworks/agent-skills` — **public and live** (`private: false`; flipped to public in the first release, `1.0.0`, and published on npm). Versioning is via **release-please** (Conventional Commits — ADR 0002, A-380), mirroring the setup in sibling repos `@acme-skunkworks/eslint-config` and `@acme-skunkworks/markdownlint-config`.
...
- **Conventional Commits.** Commits follow `<type>(<scope>?): <subject>` (e.g. `feat(cleanup-repo): add stale-branch prune step`, `chore: bump tsx`). Commit messages are convention-only (no commitlint), but the **PR title is CI-linted** by the standalone `validate-pr-title.yml` (a thin caller of `acme-skunkworks/shared-workflows`' `reusable-validate-pr-title.yml`, caller job id `pr-title`) because the repo squash-merges and that title is the squash subject release-please reads to decide the bump.
- **Draft PRs.** Open PRs as drafts by default; flip to ready when CI is green and the work is review-ready.
- **The PR title decides the release (A-380).** release-please reads the merged Conventional-Commit PR title: `feat:` → minor, `fix:`/`perf:`/`revert:` → patch, a `!`/`BREAKING CHANGE:` → major; `docs`/`chore`/`ci`/`refactor`/`test`/`build`/`style` cut no release. `/send-it` maps shippable changes to `feat`/`fix`/`feat!` only; release-please still bumps on a manually titled `perf:` or `revert:`. A release-triggering title MUST carry a dated `changelog/*.md` entry — the `📓 Changelog completeness` gate in the local `skills` job (`🧩 Skills & changelog`) enforces it (no entry → the job fails). There are no `.changeset/*.md` files any more. `/send-it` composes the right title from the branch's commits (shippable → `feat`/`fix`/breaking; non-shippable → a non-release type).
...
`/send-it` (the [`send-it` skill](skills/send-it/SKILL.md), dogfooded via the thin `.claude/commands/send-it.md` shim) is the all-in-one finisher: it commits uncommitted work as atomic Conventional Commits, runs the change-gated lint `preflight`, composes a **Conventional Commits PR title** (the release-please bump signal) and writes or updates a dated `changelog/<ts>-<slug>.md` entry for shippable changes, pushes the branch, opens or updates a draft PR, and transitions linked Linear issues to **In Review**. It is a thin orchestrator that delegates the commit step, lint gate, changelog authoring, and Linear writeback to the standalone `commit` / `preflight` / `changelog` / `linear-sync` skills. Prefer it over hand-rolled `git commit` + `git push` + `gh pr create` flows. For just the commit step on its own — atomic Conventional Commits with the out-of-scope guard, no push/PR/changelog/Linear — use the standalone [`/commit`](skills/commit/SKILL.md) (dogfooded via `.claude/commands/commit.md`).
...
3. The orchestrator waits for the required **`GO/NO GO`** check (the `go-no-go` aggregator job in `validate.yml`, which `needs:` the `config`/`lint`/`build-test`/`skills`/`skill-manifests` jobs and deliberately runs on the release PR), then squash-merges it (pinned to the gate-checked head SHA — A-334).
4. That merge re-fires `pkg-release.yml`. The version-vs-tag gate now sees a freshly bumped, **untagged** version → it runs the publish path: the build leg packs the tarball, then the npm-OIDC leg + the GitHub-Packages leg (provenance attestation) publish that exact artifact, plus an explicit idempotent git tag / GitHub release (notes sourced from the dated `changelog/` entry stamped with that version). The package is public, so both legs publish for real. A `🚨 Notify on release failure` step opens/updates a tracking issue if any step fails (the run is unattended).
...
4. **Configure the npm Trusted Publishing allowlist** at `npmjs.com/package/@acme-skunkworks/agent-skills/access` → GitHub Actions. The caller workflow filename (`pkg-release.yml`) and the `acme-skunkworks/agent-skills` repo must match the allowlist entry exactly, or OIDC publish 404s (eslint-config A-174). npm's OIDC subject matches the caller stub filename, not the reusable callee. This is only reachable _after_ step 3.
...
- **`config`** (`⚙️ Load repo config`) — local; runs `./.github/actions/load-repo-config` and exposes the repo-config knobs as outputs.
- **`lint`** — a caller of `acme-skunkworks/shared-workflows`' `reusable-lint.yml`: ESLint (over the same glob set the old `pnpm lint` covered) + Markdown (markdownlint-cli2, extending the shared `@acme-skunkworks/markdownlint-config`) + YAML/actionlint. Changelog validation is **not** here — see `skills`.
...
- **`skills`** (`🧩 Skills & changelog`) — **local**. Runs `pnpm bootstrap:config` (materialises the gitignored per-skill `config.json` from `infrastructure/dogfood-config/`, A-615), then the Vitest unit tests (`pnpm test`), skill self-tests (`pnpm test:self`), `pnpm validate:changelog`, `pnpm validate:skills` (the per-skill bundle-metadata guard, A-364), `pnpm vendor:check`, and the `📓 Changelog completeness` gate (A-380 — a `feat`/`fix`/breaking PR title must carry a dated `changelog/` entry). These stay local because they load the per-skill config at runtime and Vitest needs the `pretest` bootstrap hook the reusable caller's `vitest run` skips.
...
The Conventional-Commit PR-title lint is a separate workflow, `validate-pr-title.yml` (a thin caller of `reusable-validate-pr-title.yml`, caller job id `pr-title`), not a `validate.yml` job — the title is the release-please bump signal (A-380), so its format is linted on every PR (required status context `pr-title / Validate PR title is a Conventional Commit`).

The deeper eslint-config hardening that was deferred during the bootstrap — the build-once split + provenance attestation, the `load-repo-config` action, the dated-changelog system, and husky — **landed in [A-345](https://linear.app/acme-skunkworks/issue/A-345)** ahead of the publishing flip, and the publish path itself is now centralised in `reusable-pkg-release.yml` (A-384). The machinery is fully wired, exercised in CI, and **live** — the package is public (`private: false`), so each release-PR merge publishes for real.
...
5. Configure Trusted Publisher: `https://www.npmjs.com/package/@acme-skunkworks/agent-skills/access` → GitHub Actions → org, repo, workflow filename (`pkg-release.yml` — the caller stub, which is what npm's OIDC subject matches, not the reusable callee), environment `npm-release`.
