...
`validate.yml` has five jobs: **`🔬 Build & Lint`** (the required gate — runs on the release PR; hard-gates on the frozen-lockfile install, the vitest unit tests, `pnpm validate:changelog`, `pnpm validate:skills` (the per-skill bundle-metadata guard, ASW-364), and the `📓 Changelog completeness` gate (SK-380 — a `feat`/`fix`/breaking PR title must carry a dated `changelog/` entry)), **`yaml-lint`** (digest-pinned yamllint + actionlint), **`markdown-lint`** (markdownlint-cli2 over the tracked Markdown, extending the shared `@acme-skunkworks/markdownlint-config`), **`infra`** (shellcheck + bats over the publish scripts), and **`skill-manifests`** (validates every `skills/<name>/SKILL.md`). `yaml-lint`/`markdown-lint`/`infra`/`skill-manifests` are skipped on `release-please--*` so the release PR isn't blocked on them. The Conventional-Commit PR-title lint runs as its own **`PR Title`** workflow (`pr-title.yml`, SHA-pinned `amannn/action-semantic-pull-request`) rather than a `validate.yml` job (SK-401) — the title is the release-please bump signal (SK-380), so its format is linted on every PR (status context `Validate PR title is a Conventional Commit`).
