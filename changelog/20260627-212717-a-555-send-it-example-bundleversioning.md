...
- **`bundleVersioning` removed from `send-it/config.example.json` ([A-555](https://linear.app/rheged-studio/issue/A-555)).**
...
- **`validate-skills` key-parity check exempts config-only optional subtrees.** A
  ...
  (preserving the [A-538](https://linear.app/rheged-studio/issue/A-538) parity invariant for every other key). The exemption applies to
