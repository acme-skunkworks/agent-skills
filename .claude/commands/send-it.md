...
2. **If on `main`:**
   ...
   - If there are uncommitted changes:
     ...
     - **Branch name resolution (in order):**
       ...
       2. `--issue=<ID>` — use `<ID>-<slug>` lower-cased (e.g. `asw-7-as-acquired`), matching Linear's `gitBranchName`.
...
5. Group in-scope files into **logical atomic commits**:
   ...
   - Use Conventional Commits–style subjects: `feat:`, `fix:`, `chore:`, `docs:`, `refactor:`, `perf:`, `test:`. Include a scope when one is obvious (`feat(cleanup-repo): ...`).
