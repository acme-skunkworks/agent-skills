#!/usr/bin/env bats
# Tests for scripts/publish-to-github-packages.sh (the dormant agent-skills variant).
#
...
EOF
  ...
  echo "$output" | grep -q "skipping GitHub Packages publish (dormant)"
}
...
@test "empty-success: npm view exits 0 with no output, script publishes (not a skip)" {
  ...
  # quirk). That must be treated as "not published", not a false idempotent skip.
  write_fake_npm 0 0 ''
