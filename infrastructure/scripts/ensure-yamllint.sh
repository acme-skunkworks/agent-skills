...
# added to the `release` job — they run only in read-scoped CI jobs with no
# publish credential, which is what keeps a compromised upstream from reaching
...
# pip re-install. Mirrors ensure-bats.sh's PATH-before-check ordering (A-349).
