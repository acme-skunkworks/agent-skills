...
ADR-0002 settled _what_ is versioned — the root `@acme-skunkworks/agent-skills` as a single repo-level package, with skills versioned out-of-band via `SKILL.md metadata.version`. It assumed **Changesets** as the engine that decides the bump: every PR wrote a root-named `.changeset/*.md`, a CI guard (`validate-changesets.ts`) rejected skill-named changesets, and the private `release-orchestrator` ran `changeset version` to cut the version PR.
